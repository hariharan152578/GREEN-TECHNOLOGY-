...
const YoutubeSection: React.FC = () => {
  ...
  const selectedTestimonial = testData.find(t => t.id === selectedId);

  const [currentIndex, setCurrentIndex] = useState(0);
  const [direction, setDirection] = useState(0);
  const [visibleCards, setVisibleCards] = useState(3);
  const CARD_COUNT = testData.length;
  ...
  const visibleMaterials = Array.from({ length: visibleCards }, (_, i) => 
    testData[(currentIndex + i) % CARD_COUNT]
  );

  const getModalAspectRatio = () => {
    if (!selectedTestimonial) return 'aspect-video'; 
    const url = selectedTestimonial.videoUrl;
    if (url.includes("/shorts/") || (url.includes("youtu.be/") && url.length < 30)) return 'aspect-[9/16]'; 
    return 'aspect-video'; 
  };
  ...
  const getModalMaxWidth = () => {
    ...
    const url = selectedTestimonial.videoUrl;
    if (url.includes("/shorts/") || (url.includes("youtu.be/") && url.length < 30)) return 'max-w-xs md:max-w-sm'; 
        ...
        <div className="relative flex items-center justify-center">
          ...
          <div className="relative overflow-hidden w-full min-h-[400px]">
            <AnimatePresence initial={false} custom={direction} mode="popLayout">
              ...
              >
                {visibleMaterials.map((testimonial) => (
                  <motion.div
                    key={testimonial.id}
